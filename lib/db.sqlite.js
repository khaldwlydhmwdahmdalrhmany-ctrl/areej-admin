...
export async function updateProduct(id, data) {
  ...
  db.prepare(`
    UPDATE products SET name=?, description=?, fullDescription=?, price=?, oldPrice=?, badge=?, imageUrl=?, freeShipping=?, freeInstall=?, categoryId=?, published=?, brand=?, stock=?, rating=?, reviewCount=?, updatedAt=datetime('now')
    WHERE id=?
  `).run(
...
export async function createOrder({ customerName, customerPhone, customerCity, items, total }) {
  ...
  const id = newId();
  db.prepare(`
    INSERT INTO orders (id, orderNumber, customerName, customerPhone, customerCity, itemsJson, total) VALUES (?, ?, ?, ?, ?, ?, ?)
